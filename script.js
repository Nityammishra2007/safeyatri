 ...
 * automatically, on top of whatever "Download Offline Map" has
 * already saved.
 */
...
const BHUBANESWAR_BOUNDS_ARRAY = [
    [20.15, 85.68],
    [20.43, 85.98]
];
...
const OFFLINE_DOWNLOAD_ZOOMS = [11, 12, 13, 14];
...
function tileRangeForBhubaneswar(z) {

    const south = BHUBANESWAR_BOUNDS_ARRAY[0][0];
    const west = BHUBANESWAR_BOUNDS_ARRAY[0][1];
    const north = BHUBANESWAR_BOUNDS_ARRAY[1][0];
    const east = BHUBANESWAR_BOUNDS_ARRAY[1][1];

    return {
        minX: lonToTileX(west, z),
        maxX: lonToTileX(east, z),
        minY: latToTileY(north, z),
        maxY: latToTileY(south, z)
    };
}

function buildBhubaneswarTileList() {

    const tiles = [];

    OFFLINE_DOWNLOAD_ZOOMS.forEach(z => {

        const range = tileRangeForBhubaneswar(z);
...
function setOfflineMapStatus(html) {

    const statusEl =
        document.getElementById("offlineMapStatus");
...
async function downloadOfflineBhubaneswarMap() {

    if (offlineDownloadRunning) return;

    if (!("serviceWorker" in navigator) || !("caches" in window)) {
        setOfflineMapStatus(`
            <span class="offline-map-status-text" style="color:#f59e0b;">
                Offline maps aren't supported in this browser.
            </span>
        `);
        return;
    }
    ...
    try {
        ...
        const reason =
            !window.isSecureContext
                ? "this page isn't loaded over HTTPS (service workers need a secure connection)."
                : "sw.js couldn't be reached — make sure it's deployed at your site's root.";

        setOfflineMapStatus(`
            <span class="offline-map-status-text" style="color:#f87171;">
                Couldn't set up offline storage: ${reason}
            </span>
        `);
    ...
    const tiles = buildBhubaneswarTileList();
    ...
    for (const tile of tiles) {
        ...
        const subdomain =
            TILE_SUBDOMAINS[
                (tile.x + tile.y) % TILE_SUBDOMAINS.length
            ];
        ...
        if (done % 15 === 0 || done === tiles.length) {
            ...
            setOfflineMapStatus(`
                <div class="offline-map-progress-track">
                    <div class="offline-map-progress-fill"
                         style="width:${pct}%;"></div>
                </div>
                <span class="offline-map-status-text">
                    Saving Bhubaneswar map for offline use… ${pct}%
                </span>
            `);
        }
    ...
    if (failed === 0) {
        ...
        setOfflineMapStatus(`
            <span class="offline-map-status-text" style="color:#16a34a;">
                ✅ Bhubaneswar map saved for offline use
                (${tiles.length} map tiles).
            </span>
        `);

        showToast("Offline map ready — Bhubaneswar will load with no signal.");

    } else {

        setOfflineMapStatus(`
            <span class="offline-map-status-text" style="color:#f59e0b;">
                Saved ${tiles.length - failed} of ${tiles.length} tiles.
                Some couldn't download — try again on a better connection.
            </span>
        `);

        showToast("Offline map partially saved.");
    }
}
